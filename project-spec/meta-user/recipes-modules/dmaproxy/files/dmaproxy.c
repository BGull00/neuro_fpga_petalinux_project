 ...
 *
 * There is an associated user space application, dmaproxy_test.c, and dmaproxy.h
 ...
 * access for each DMA channel, such as /dev/dmaproxy_rx and /dev/dmaproxy_tx.
 ...
 *   dma-names = "dmaproxy_tx_0", "dmaproxy_rx_0", "dmaproxy_tx_1", "dmaproxy_rx_1";
 * };
 */
...
struct dmaproxy {
	int channel_count;
	struct dmaproxy_channel *channels;
	char **names;
 ...
 * transaction
 */
static void start_transfer(struct dmaproxy_channel *pchannel_p)
{
...
{
	struct dmaproxy_channel *pchannel_p = (struct dmaproxy_channel *)file_p->private_data;
...
{
	file->private_data = container_of(ino->i_cdev, struct dmaproxy_channel, cdev);
...
{
#if 0
	struct dmaproxy_channel *pchannel_p = (struct dmaproxy_channel *)file->private_data;
	struct dma_device *dma_device = pchannel_p->channel_p->device;
 ...
 * file operations.
 */
static int cdevice_init(struct dmaproxy_channel *pchannel_p, char *name)
{
	int rc;
	char device_name[32] = "dmaproxy";
	static struct class *local_class_p = NULL;

	/* Allocate a character device from the kernel for this driver.
	 */
	rc = alloc_chrdev_region(&pchannel_p->dev_node, 0, 1, "dmaproxy");
 ...
 */
static void cdevice_exit(struct dmaproxy_channel *pchannel_p)
{
	...
	if (pchannel_p->proxy_device_p) {
		...
		/* If this is the last channel then get rid of the /sys/class/dmaproxy
		 */
		if (total_count == 1)
			class_destroy(pchannel_p->class_p);
 ...
 */
static int create_channel(struct platform_device *pdev, struct dmaproxy_channel *pchannel_p, char *name, u32 direction)
{
	int rc, bd;
...
static int dmaproxy_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "dmaproxy module initialized\n");
	
	lp = (struct dmaproxy *) devm_kmalloc(&pdev->dev, sizeof(struct dmaproxy), GFP_KERNEL);
	if (!lp) {
    ...
    * from the device tree
	...
	lp->channels = devm_kmalloc(&pdev->dev,
			sizeof(struct dmaproxy_channel) * lp->channel_count, GFP_KERNEL);
	if (!lp->channels)
		return -ENOMEM;
...
static int dmaproxy_remove(struct platform_device *pdev)
{
	int i;
	struct device *dev = &pdev->dev;
	struct dmaproxy *lp = dev_get_drvdata(dev);

	printk(KERN_INFO "dmaproxy module exited\n");
...
static const struct of_device_id dmaproxy_of_ids[] = {
	{ .compatible = "xlnx,dmaproxy",},
	{}
};
...
static struct platform_driver dmaproxy_driver = {
	.driver = {
		.name = "dmaproxy_driver",
		.owner = THIS_MODULE,
		.of_match_table = dmaproxy_of_ids,
	},
...
{
	return platform_driver_register(&dmaproxy_driver);
