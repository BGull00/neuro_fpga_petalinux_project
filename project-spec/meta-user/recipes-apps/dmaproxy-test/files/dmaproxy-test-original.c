 ...
 * tree.
 *
 * More complete documentation is contained in the device driver (dmaproxy.c).
 */
...
const char *tx_channel_names[] = { "dmaproxy_tx", /* add unique channel names here */ };
const char *rx_channel_names[] = { "dmaproxy_rx", /* add unique channel names here */ };
...
int main(int argc, char *argv[])
{
	...
	if ((argc != 3) && (argc != 4)) {
		printf("Usage: dmaproxy-test <# of DMA transfers to perform> <# of bytes in each transfer in KB (< 1MB)> <optional verify, 0 or 1>\n");
		exit(EXIT_FAILURE);
	}
